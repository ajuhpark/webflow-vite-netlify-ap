...
/** * This one from chat gpt actually works
...
function init(){
  try {
    ...
    if (typeof SplitText !== "function") {
      console.error("SplitText is not defined or not loaded correctly.");
      return;
    }
    
    split = new SplitText(".home_aboutSection_text_1", {type: "chars"});
    console.log("Text split into characters:", split.chars);

    home_tl
      .from(split.chars, {duration: 1, yPercent: 100, opacity: 0, stagger: 0.05});
...
document.addEventListener('DOMContentLoaded', (event) => {
  let split = new SplitText(".home_aboutSection_text_1", {type: "chars"})

  gsap.from(split.chars, {duration: 1, yPercent: 100, opacity: 0, stagger: 0.05})
          .from('.home_aboutSection_text_1', { yPercent: 110, duration: 1.1 }, 'home_start+=.3')
  
})

document.addEventListener('DOMContentLoaded', () => {
  // Check if SplitText is defined
  if (typeof SplitText !== 'undefined') {
    ...
    function init(){
      home_tl
        ...
        // .from('.home_name', { opacity: 0, duration: .9 }, 'home_start+=.3' )
        split = new SplitText('.home_name', {type:"chars"});
        home_tl.from(split.chars, {opacity:0, y:50, ease:"back(4)", stagger:0.5})
    ...
    console.error("SplitText is not defined");
  }
});
