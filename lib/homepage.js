...
// document.addEventListener('DOMContentLoaded', (event) => {
  ...
  function init(){
    ...
    //this works but the split cars is getting an error.
      ...
      .from(split_name.chars, {yPercent: 100, opacity: 0, stagger: 0.01, duration: .5, ease:"circ.out"}, 'home_start+=.3')
      .from(split1.chars, {yPercent: 90, opacity: 0, stagger: 0.012, duration: .3, ease:"circ.out"}, 'home_start+=.4')
      .from(split2.chars, {yPercent: 80, opacity: 0, stagger: 0.012, duration: .4,  ease:"circ.out"}, 'home_start+=.5')
